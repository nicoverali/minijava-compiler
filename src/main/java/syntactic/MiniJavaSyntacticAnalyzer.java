...
public class MiniJavaSyntacticAnalyzer implements SyntacticAnalyzer {
	...
	private void listaMiembros() {
		if (equalsAny(K_PRIVATE, K_PUBLIC, K_DYNAMIC, ID_CLS, K_STATIC, K_INT, K_BOOLEAN, K_CHAR, K_STRING)) {
			miembro();
			listaMiembros();
	...
	private void miembro() {
		if (equalsAny(K_PRIVATE, K_PUBLIC)) {
			...
			match(K_STATIC);
			atributoOMetodo();
		} else if (equalsAny(K_INT, K_BOOLEAN, K_CHAR, K_STRING)) {
			tipoPrimitivo();
			listaDecAtrs();
			match(P_SEMICOLON);
		} else {
	...
	private void atributoOMetodo() {
		if (equalsAny(K_INT, K_BOOLEAN, K_CHAR, K_STRING, ID_CLS)) {
			tipo();
			match(ID_MV);
	...
	private void listaSentencias() {
		if (equalsAny(K_INT, K_BOOLEAN, K_CHAR, K_STRING, K_RETURN, K_IF, P_BRCKT_OPEN, K_THIS, ID_MV, K_NEW, P_PAREN_OPEN, K_FOR, P_SEMICOLON, ID_CLS)) {
			sentencia();
			listaSentencias();
		} else if (equalsAny(P_BRCKT_CLOSE)) { // Check for follow
			// Nothing for now
	...
	private void sentencia() {
		if (equalsAny(K_INT, K_BOOLEAN, K_CHAR, K_STRING)) {
			varLocalPrimitiva();
			match(P_SEMICOLON);
			...
			bloque();
		} else if (equalsAny(K_THIS, ID_MV, K_NEW, P_PAREN_OPEN)) {
	...
	private void expresionOVacio() {
		if (equalsAny(OP_NOT, OP_MINUS, OP_PLUS, K_TRUE, INT, K_FALSE, CHAR, STRING, K_NULL, K_THIS, ID_MV, K_NEW, P_PAREN_OPEN, ID_CLS)) {
			expresion();
		} else if (equalsAny(P_SEMICOLON)) { // Check for follow
			// Nothing for now
		} else {
			sequence.peek().ifPresent(token -> 
				{throw new SyntacticException("Se esperaba {!, new, intLiteral, false, this, (, charLiteral, +, -, idMetVar, null, stringLiteral, true, idClase, ;} pero se encontro "+token.getLexeme()+" ("+token.getType()+")", token);});
		}
	}
	...
	private void expresionUnaria() {
		if (equalsAny(OP_NOT, OP_MINUS, OP_PLUS, K_TRUE, INT, K_FALSE, CHAR, STRING, K_NULL, K_THIS, ID_MV, K_NEW, P_PAREN_OPEN)) {
	...
	private void expresionUnariaSinStatic() {
		if (equalsAny(OP_NOT, OP_MINUS, OP_PLUS)) {
			operadorUnario();
			operando();
		} else if (equalsAny(K_TRUE, INT, K_FALSE, CHAR, STRING, K_NULL, K_THIS, ID_MV, K_NEW, P_PAREN_OPEN)) {
			operando();
		} else {
			sequence.next().ifPresent(token -> 
	...
	private void operando() {
		if (equalsAny(K_TRUE, INT, K_FALSE, CHAR, STRING, K_NULL)) {
			literal();
		} else if (equalsAny(K_THIS, ID_MV, K_NEW, P_PAREN_OPEN)) {
			acceso();
		} else {
	...
	private void acceso() {
		if (equalsAny(K_THIS, ID_MV, K_NEW)) {
			primario();
			encadenado();
		} else if (equalsAny(P_PAREN_OPEN)) {
			match(P_PAREN_OPEN);
	...
	private void expParenOCasting() {
		...
		} else if (equalsAny(OP_NOT, OP_MINUS, OP_PLUS, K_TRUE, INT, K_FALSE, CHAR, STRING, K_NULL, K_THIS, ID_MV, K_NEW, P_PAREN_OPEN)) {
			expresionUnariaSinStatic();
			expresionResto();
			match(P_PAREN_CLOSE);
		} else {
	...
	private void expresionResto() {
		if (equalsAny(OP_MINUS, OP_PLUS, OP_LTE, OP_DIV, OP_GT, OP_LT, OP_EQ, OP_MOD, OP_AND, OP_NOTEQ, OP_OR, OP_GTE, OP_MULT)) {
			operadorBinario();
			expresionUnaria();
			expresionResto();
	...
	private void tipoMetodo() {
		if (equalsAny(K_VOID)) {
			match(K_VOID);
		} else if (equalsAny(K_INT, K_BOOLEAN, K_CHAR, K_STRING, ID_CLS)) {
			tipo();
		} else {
			sequence.next().ifPresent(token -> 
